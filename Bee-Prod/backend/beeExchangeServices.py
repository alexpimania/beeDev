...
def extractListFromText(listName, text, listOrString):
    #Finds the position of the list in the text by locating two peices of text that signify the beginning and end of the list
    header = "<" + listName + ">"
    footer = "</" + listName + ">"
    delimiter = ";"

    if header in text and footer in text:
        if listOrString == "list":
            beginingIndexOfListText = text.index(header) + len(header)
            endingIndexOfListText = text.index(footer)
            #Extracts and parses the list of entries
            rawList = text[beginingIndexOfListText : endingIndexOfListText]
            foundList = rawList.strip().strip(delimiter).replace("\r\n", "").split(delimiter)
            return foundList
            
        if listOrString == "string":
            beginingIndexOfListText = text.index(header)
            endingIndexOfListText = text.index(footer) + len(footer)
            rawList = text[beginingIndexOfListText : endingIndexOfListText]
            return rawList
...
def getRecentImprovments():
    import json
    import os
    if os.path.exists(BEE_PATH + "/backend/improvementsList.ini"):
        improvementsFile = open(BEE_PATH + "/backend/improvementsList.ini")
        improvementsList = extractListFromText("improvements", improvementsFile.read().strip(), "list")
    else:
        improvementsList = []
        
    return json.dumps(improvementsList)
    

print(getRecentImprovments())
